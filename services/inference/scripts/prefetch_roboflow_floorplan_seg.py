"""Download / cache Roboflow floorplan-segmentation-imdze weights for local inference.

Universe: https://universe.roboflow.com/floorplan-lfnvy/floorplan-segmentation-imdze
Classes: wall, door, window (instance segmentation).

Usage (Python 3.10–3.12 with ``inference`` installed, e.g. .venv-tf):

  .venv-tf\\Scripts\\python.exe scripts/prefetch_roboflow_floorplan_seg.py
"""
...
DEFAULT_MODEL = "floorplan-segmentation-imdze/4"


def main() -> int:
    key = (os.getenv("ROBOFLOW_API_KEY") or "").strip()
    model_id = (os.getenv("ROBOFLOW_FLOORPLAN_SEG_MODEL_ID") or "").strip() or DEFAULT_MODEL
    ...
    print(f"Cache: {CACHE}")
    print(f"Loading {model_id} (wall / door / window instance seg)…")
    try:
    ...
    project = model_id.split("/", 1)[0]
    marker = CACHE / f"{project}.ready"
